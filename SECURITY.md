...
- Rich delivery details use a separate personal Owner authorization with PKCE. OrderPulse accepts only the exact `https://auth.tesla.com/void/callback` origin/path, consumes `state` once within one hour, never receives the Tesla password or MFA code, and encrypts the resulting tokens on the NAS.
